...
var clickHandler = function(targetElement){
    ...
    if(playingSongNum === null){
        ...
        playingSongNum = songItem.getAttribute('data-song-number');
    }else if(playingSongNum === songItem.getAttribute('data-song-number')){
        songItem.innerHTML = playButtonTemplate
        playingSongNum = null;
    }else if(playingSongNum !== songItem.getAttribute('data-song-number')){
        var currentlyPlayingSongElement = document.querySelector('[data-song-number="' + playingSongNum + '"]');
...
window.onload = function(){
    ...
    songListContainer.addEventListener("mouseover", function(event){
        if (event.target.parentElement.className === 'album-view-song-item'){
            ...
            if(songItem.getAttribute('data-song-number') !== playingSongNum){
                songItem.innerHTML = playButtonTemplate; 
            }
        }
    });
    ...
    for(var i = 0; i < songRows.length; i++){
        songRows[i].addEventListener("mouseleave", function(event){
            ...
            if(songItemNumber != playingSongNum){
